...
public class FlightServiceImpl implements FlightService {
    ...
    public List<Flight> searchFlights(String origin, String destination) throws FlightNotFoundException {
        ...
        if (flights.isEmpty()) throw new FlightNotFoundException("No flights found from " + origin + " to " + destination);
        return flights;
    }

    @Override
    public List<Flight> searchFlightsByDate(String origin, String destination, LocalDate departureDate) throws FlightNotFoundException {
        List<Flight> flights = flightRepo.findByRouteOriginAndRouteDestinationAndDepartureDate(origin, destination, departureDate);
        if (flights.isEmpty()) throw new FlightNotFoundException("No flights found from " + origin + " to " + destination + " on " + departureDate);
        return flights;
    }
    ...
    public List<Flight> getFlightsByOwner(int ownerId) throws FlightNotFoundException {
        ...
        if (flights.isEmpty()) throw new FlightNotFoundException("No flights found for owner with ID: " + ownerId);
        return flights;
    }
}
